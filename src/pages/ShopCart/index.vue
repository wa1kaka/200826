      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="shopCart in shopCartList" :key="shopCart.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" @click="updateOne(shopCart)" :checked='shopCart.isChecked'>
          </li>
    ...
    methods:{
      ...
      async changeCartNum(shopCart,disNum,flag){
         if(!flag){
           ...
           //把传递过来的数据全部转化为变化的量之后就可以发请求了
          
       },
       //修改购物车选中状态单个的
       async updateOne(shopCart){
          try {
            await this.$store.dispatch('getUpdateCartChecked',{skuId:shopCart.skuId,isChecked:shopCart.isChecked?0:1})
            this.getCartList()
          } catch (error) {
            alert(error.message)
          }      
